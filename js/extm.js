    ...
    extm: function extm(userOptions) {
      ...
      function extmInit(options, imageElement) {
        ...
        if (!options.zoomElement) {
          zoomElement = $(
            "<div style='overflow:hidden;pointer-events:none;height:" +
              smallHeight +
              "px;width:" +
              smallWidth +
              "px;' class='extm'></div>"
          ...
          if (options.position === "right") {
            ...
            zoomElement.css(
              "left",
              offset.left + smallWidth + options.rightPad
            );
        ...
        if (options.squareOverlay) {
          overlayElement = $(
            "<div class='overlayElement' style='pointer-events:none;height:" +
              smallHeight +
              "px;width:" +
              smallWidth +
          ...
          innerOverlayElement = $(
            "<div style='background-color:rgba(0,0,0,0.2);position:absolute;' class='innerOverlay'></div>"
          );
        ...
        if (options.zoomSize) {
          var fullSizeImage = $(
            '<img style="position:relative;max-width:none;width:' +
              options.zoomSize +
              'px;">'
          ); //make a large clone and insert it
        } else if (options.zoomLevel !== 1) {
          var fullSizeImage = $(
            '<img style="position:relative;max-width:none;width:' +
              100 * options.zoomLevel +
              '%;">'
          ); //make a large clone and insert it
        } else {
          var fullSizeImage = $(
            '<img style="position:relative;max-width:none;">'
          ); //make a large clone and insert it
        }
        if (options.imageSrc) {
        ...
        (function (
          ...
          if (options.loadingText) {
            var loadingElement = $(
              "<div class='loadingElement' style='pointer-events:none;height:" +
                smallHeight +
                "px;width:" +
                smallWidth +
                "px;font-family:sans-serif;position:absolute;top:" +
                offset.top +
                "px;left:" +
                offset.left +
                "px;visibility:hidden;background:rgba(255,255,255,0.5);color:black;font-size:2em;font-weight:bold;text-align:center;'><p style='position: absolute; left: 50%;top: 50%;transform: translate(-50%, -50%);'>" +
                options.loadingText +
                "</p></div>"
            );
            ...
            var loadingElement = $(
              "<div class='loadingElement' style='pointer-events:none;height:" +
                ...
                "px;left:" +
                offset.left +
                "px;visibility:hidden;background:rgba(255,255,255,0.5);color:black;font-size:2em;font-weight:bold;text-align:center;'><img style='position: absolute; left: 50%;top: 50%;transform: translate(-50%, -50%);' src='" +
          ...
          fullSizeImage.on("load", function () {
            ...
            imageElement.on("mousemove", function (event) {
              //on mousemove, use ratios and heights to move appropriately
              offset = imageElement.offset();
              var setTop =
                smallHeight / 2 - (event.pageY - offset.top) * hRatio;
              setTop = Math.max(setTop, hDifference);
              setTop = Math.min(setTop, 0);
              var setLeft =
                smallWidth / 2 - (event.pageX - offset.left) * wRatio;
              ...
              if (options.squareOverlay) {
                var squareTop =
                  event.pageY - offset.top - innerOverlayElement.height() / 2;
                squareTop = Math.max(squareTop, 0);
                squareTop = Math.min(
                  squareTop,
                  smallHeight - innerOverlayElement.height()
                );
                var squareLeft =
                  event.pageX - offset.left - innerOverlayElement.width() / 2;
                squareLeft = Math.max(squareLeft, 0);
                squareLeft = Math.min(
                  squareLeft,
                  smallWidth - innerOverlayElement.width()
                );
